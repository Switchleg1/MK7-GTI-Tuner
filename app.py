...
from geometry import build_car, make_box, make_grid
from simos import build_context, select_insight
from tuning import clone_tune, compute_tune, default_tune, dyno_curve, grade_for_result, pop_score, rep_title
...
class MK7Tuner3D(ShowBase):
    def __init__(self):
        ...
        self.log("Camera FOV 105 deg, default aspect 16:9.", "info")
        self.log("Connect -> read -> patch -> flash.", "dim")
    ...
    def draw_bench_ui(self, left, right):
        ...
        self.ui_label("SIMOSTOOLS - FLASH SEQUENCE", (lbox[0] + 0.05, 0, 0.40), 0.044, BLUE)
        self.ui_label("Plug into OBD, read the calibration, unlock writing with the ECM3 boot patch, then flash.", (lbox[0] + 0.05, 0, 0.29), 0.034, TEXT, wordwrap=24)
        for text, x, enabled, command, color in [
            ("1 Connect OBD", 0.23, not self.connected, self.connect_obd, GREEN_2),
            ("2 Read ECU", 0.64, self.connected and not self.read, self.read_ecu, None),
            ("3 Boot Patch", 1.02, self.read and not self.patched, self.patch_ecu, None),
        ]:
            self.ui_button(text, (lbox[0] + x, 0, 0.12), (0.36, 0.11), command, enabled, color)
        self.ui_button(f"switch patch: {'ON' if self.switch_patch else 'OFF'}", (lbox[0] + 0.32, 0, -0.06), (0.48, 0.10), self.toggle_switch, self.patched)
        self.ui_button("4 FLASH ECU", ((lbox[0] + lbox[1]) / 2, 0, -0.28), (lbox[1] - lbox[0] - 0.12, 0.12), self.flash_ecu, self.patched, GREEN_2)
        if self.dirty:
            self.ui_label("Flash required for changed tune.", (lbox[0] + 0.05, 0, -0.42), 0.033, AMBER)
    ...
    def patch_ecu(self):
        ...
        self.log("write access UNLOCKED.", "ok")
