...
class BenchTask(TaskBase):
    ...
    def build_ui(self, left, right):
        ...
        self.label(f"Loaded tune: {game.tune.get('name', 'Your Tune')}", (lbox[0] + 0.05, 0, 0.16), 0.036, TEXT)
        self.button(f"switch patch: {'ON' if game.switch_patch else 'OFF'}", (lbox[0] + 0.32, 0, 0.02), (0.48, 0.10), self.bind(game.toggle_switch))
        self.button("FLASH ECU", ((lbox[0] + lbox[1]) / 2, 0, -0.24), (lbox[1] - lbox[0] - 0.12, 0.12), self.bind(game.flash_ecu), True, GREEN_2)
        if game.dirty:
