...
class ShopTask(TaskBase):
    ...
    def build_ui(self, left, right):
        ...
        self.label(f"SHOP  -  CASH ${game.cash}", (left + 0.18, 0, 0.40), 0.044, BLUE)
        for index, (mod_id, name, cost, _desc) in enumerate(MODS):
            row, col = divmod(index, 2)
            x = left + 0.28 + col * 0.82
            y = 0.25 - row * 0.12
            label = f"{name} - {'owned' if game.mods[mod_id] else '$' + str(cost)}"
            self.button(label, (x, 0, y), (0.72, 0.08), self.bind(game.buy_mod, mod_id), not game.mods[mod_id] and game.cash >= cost)
