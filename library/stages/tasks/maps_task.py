...
class MapsTask(TaskBase):
    ...
    def build_ui(self, left, right):
        ...
        lines = [f"Boost: {car.tune['boost']:.1f} psi", f"Timing: {car.tune['timing']:.1f} deg", f"Lambda: {car.tune['lambda']:.3f}", f"Fuel: {car.tune['fuel']}"]
        ...
        self.label(f"Burble index: {round(pop_score(car.tune))}", (rbox[0] + 0.19, 0, 0.27), 0.05, AMBER)
        y = 0.14
        for index, slot in enumerate(car.slots):
            ...
            y -= 0.10
        self.button("Assign Current Tune", (rbox[0] + 0.36, 0, -0.34), (0.58, 0.10), self.bind(game.assign_slot), car.flashed)
